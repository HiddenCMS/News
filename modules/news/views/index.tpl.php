...
<?php if ($image): ?>
	<a href="<?php echo url($news_module->news_path($category_name, $title, isset($slug) ? $slug : '')) ?>">
		<img class="card-img-top" src="<?php echo HB()->model2('file', $image)->path() ?>" alt="" />
	</a>
<?php endif ?>
<div class="card-body">
	<h5 class="card-title"><a href="<?php echo url($news_module->news_path($category_name, $title, isset($slug) ? $slug : '')) ?>"><?php echo $title ?></a></h5>
	...
	<blockquote class="blockquote mb-0">
		...
		<?php endif ?>
		<footer class="blockquote-footer"><?php echo $this->lang('By').' '.($user_id ? $this->user->link($user_id, $username) : ($username ?: $this->lang('Visitor'))).' '.$this->lang('on').' '.timetostr('j M Y', $date) ?> / <a href="<?php echo url($news_module->category_path($category_name)) ?>"><?php echo $category_title ?></a><?php echo (($comments = $this->module('comments')) && $comments->is_enabled()) ? ' / '.$comments->link('news', $news_id, $news_module->news_path($category_name, $title, isset($slug) ? $slug : '')) : '' ?></footer>
	</blockquote>
</div>
<?php if ($tags || $content): ?>
	...
	<?php if ($tags): ?>
		<ul class="list-inline mb-0 float-left">
			...
			<?php foreach (explode(',', $tags) as $tag): ?>
				<li class="list-inline-item"><a href="<?php echo url($news_module->tag_path($tag)) ?>"><small><?php echo $tag ?></small></a></li>
			<?php endforeach ?>
		</ul>
	<?php endif ?>
	<?php if ($content): ?>
		<a href="<?php echo url($news_module->news_path($category_name, $title, isset($slug) ? $slug : '')) ?>" class="btn btn-sm btn-secondary float-right"><?php echo $this->lang('Continue reading') ?></a>
	<?php endif ?>
</div>
<?php endif ?>
