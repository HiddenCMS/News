...
{
	public function page_block($block, $settings = [])
	{
		$category_id = strpos($block, 'category:') === 0 ? (int)substr($block, 9) : (int)(isset($settings['category_id']) ? $settings['category_id'] : 0);
		...
		$news = $this->model()->get_news($category_id ? 'category' : '', $category_id);

		$this->css('page-blocks');
